...
function Update () {
	...
	if(Ring_Rotate_2.play_2 && !Instructions.instructions && GameObject.Find("Pause Plane").transform.position.z == 10){
		...
		if(Ring_Rotate.mode_2 && !Ring_Rotate.mode_1){
			...
			if(GameObject.Find("Green Pointer 2 Empty").transform.rotation.y == 0){
			
				for(var i = 0; i < Ring_Rotate_2.score_rings_2.length; i++){
				
					if(Ring_Rotate_2.score_rings_2[i] != "Green Score Ring"){
				...
				for(var j = 0; j < Ring_Rotate_2.pause_rings_2.length; j++){
					...
					if(Ring_Rotate_2.pause_rings_2[j] != "Green Pause Ring"){
				...
				if(Ring_Rotate_2.clockwise_2){
					...
					GameObject.Find("Green Pointer 2 Empty").transform.Rotate(Vector3.back * Ring_Rotate_2.speed_2);
					
				}else{
				
					GameObject.Find("Green Pointer 2 Empty").transform.Rotate(Vector3.forward * Ring_Rotate_2.speed_2);
			...
			if(Ring_Rotate_2.check_2){
				...
				if(Input.GetMouseButtonDown(0)){
					...
					if(Physics.Raycast(ray, hit)){
							...
							if(hit.transform.name not in Ring_Rotate.intangible_items){
								...
								if(GameObject.Find("Green Pointer 2 Empty").transform.rotation.y == 0){
									...
									if(Ring_Rotate_2.play_2){
										...
										Ring_Rotate_2.Clockwise_Anticlockwise_2("Green Pointer 2", "Green Pointer 2 Empty", "Green Side 2", "Green Pause Ring");
						...
						if(GameObject.Find("Green Pause Ring").transform.position.x == 0){
							...
							if(hit.transform.name in Play_Pause.restart_items){
								...
								if(GameObject.Find("Restart Symbol").transform.position.x == 0){
									...
									Ring_Rotate_2.Restart_2("Green Pointer 2 Empty", "Green Pause Ring");
				...
				if(Ring_Rotate_2.play_2 && Ring_Rotate.mode_2 && !Instructions.instructions){
					...
					if(GameObject.Find("Green Pause Ring").transform.position.x == 0){
						...
						if(Ring_Rotate_2.Distance_2("Green Pointer 2", "Green Side 2", "Green Pause Ring")){
						...
						if(Ring_Rotate_2.in_range_2){
							...
							if(!Ring_Rotate_2.Distance_2("Green Pointer 2", "Green Side 2", "Green Pause Ring")){
								...
								Ring_Rotate_2.Game_Over_2("Green Pointer 2", "Green Pointer 2 Empty", "Green Pause Ring");
